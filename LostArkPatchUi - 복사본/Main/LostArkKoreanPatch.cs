...
namespace LostArkKoreanPatch
{
    public partial class LostArkKoreanPatch : Form
    {
        ...
        private string[] distribFileNames = new string[]
        {
            "font",
            "orig/font",
            "data2",
        ...
        // Downloads a file from given url and return the response.
        private byte[] DownloadFile(string url, string fileName, BackgroundWorker backgroundWorker)
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                Invoke(new Action(() =>
                {
                    statusLabel.Text = $"다운로드중... {fileName}";
                }));
                ...
                using (HttpResponseMessage responseMessage = client.GetAsync(url, HttpCompletionOption.ResponseHeadersRead).GetAwaiter().GetResult())
                {
                    if (responseMessage.Content.Headers.ContentLength != null)
                    {
                        ...
                        // Create a memory stream for destination and grab stream from client.
                        ...
                        using (Stream inStream = responseMessage.Content.ReadAsStreamAsync().GetAwaiter().GetResult())
                        {
                            ...
                            // Reset the label since download is completed.
                            Invoke(new Action(() =>
                            {
                                statusLabel.Text = $"다운로드 완료. {fileName}";
                            }));
            ...
            // If anything happened and didn't reach download success, return null.
            return null;
        }
        ...
        // Shared progress bar for all background workers.
